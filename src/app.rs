use crate::cli::{
    Cli, CloseArgs, Commands, ConnectArgs, DeleteArgs, ExposeArgs, ForwardArgs, ListArgs, LogsArgs,
    RestartArgs, StatusArgs, StopArgs, TunnelArgs, TunnelCommands,
};
use crate::error::ConduitError;
use crate::expose;
use crate::forward::{self, ForwardConfig, ForwardMapping};
use crate::tunnel_registry::{self, TunnelMode};
use anyhow::Context;
...
#[derive(Debug, Clone)]
pub enum PreparedTunnelConfig {
    Reverse(expose::types::ExposeConfig),
...
pub struct PreparedDaemonCommand {
    pub command: String,
    pub config: PreparedTunnelConfig,
...
pub async fn run(cli: Cli) -> anyhow::Result<()> {
    match cli.command {
        ...
        Commands::Forward(args) => run_forward(args).await,
...
pub async fn run_prepared_daemon(prepared: PreparedDaemonCommand) -> anyhow::Result<()> {
    match prepared.config {
        PreparedTunnelConfig::Reverse(config) => {
            run_reverse_tunnel_command(&prepared.command, config, Some(prepared.launch)).await
        }
        PreparedTunnelConfig::Forward(config) => {
            run_forward_tunnel_command(&prepared.command, config, Some(prepared.launch)).await
        }
    }
}
...
async fn run_connect(args: ConnectArgs) -> anyhow::Result<()> {
    ...
    run_reverse_tunnel_command("connect", config, None).await
}

async fn run_expose(args: ExposeArgs) -> anyhow::Result<()> {
    let config = expose::types::ExposeConfig::try_from(args)?;
    run_reverse_tunnel_command("expose", config, None).await
}

async fn run_forward(args: ForwardArgs) -> anyhow::Result<()> {
    let config = ForwardConfig::try_from(args)?;
    run_forward_tunnel_command("forward", config, None).await
}
...
fn run_list(args: ListArgs) -> anyhow::Result<()> {
    ...
    println!("ID\tMODE\tSTATUS\tPID\tREMOTE\tLOCAL");
    for record in records {
        println!(
            ...
            record.pid,
            record.display_remote(),
            record.display_local()
        );
    }
...
fn run_status(args: StatusArgs) -> anyhow::Result<()> {
    ...
    match record.mode {
        ...
        TunnelMode::Forward => {
            println!("bind: {}", record.bind_addr.as_deref().unwrap_or("unknown"));
            println!("mappings:");
            for mapping in &record.forward_mappings {
                println!("  - {}", mapping);
            }
        }
    }
...
pub fn prepare_daemon_command(cli: &Cli) -> anyhow::Result<Option<PreparedDaemonCommand>> {
    match &cli.command {
        Commands::Connect(args) if args.daemon => {
            ...
            let launch = prepare_daemon_launch(config.log_file.as_ref(), None)?;
            Ok(Some(PreparedDaemonCommand {
                command: "connect".to_string(),
                config: PreparedTunnelConfig::Reverse(config),
                launch,
        ...
        Commands::Forward(args) if args.daemon => {
            let config = ForwardConfig::try_from(args.clone())?;
            let launch = prepare_daemon_launch(config.log_file.as_ref(), None)?;
            Ok(Some(PreparedDaemonCommand {
                command: "forward".to_string(),
                config: PreparedTunnelConfig::Forward(config),
                launch,
                close_before_launch: None,
            }))
        }
...
async fn run_reverse_tunnel_command(
    command: &str,
...
async fn run_forward_tunnel_command(
    command: &str,
    mut config: ForwardConfig,
    prepared_daemon: Option<PreparedDaemonLaunch>,
) -> anyhow::Result<()> {
    let tunnel_id = if config.daemon {
        let prepared = prepared_daemon.context("missing prepared daemon launch")?;
    ...
    info!(
        ...
        daemon = config.daemon,
        "starting forward tunnel command"
    );

    match forward::run(config.clone(), tunnel_id.clone()).await {
        Ok(()) => {
            if let Some(id) = tunnel_id.as_deref() {
                let _ = tunnel_registry::mark_exited(id);
            }
...
fn build_restart_config(
    record: &tunnel_registry::TunnelRecord,
    args: RestartArgs,
) -> anyhow::Result<PreparedTunnelConfig> {
    if !record.daemon {
    ...
    match record.mode {
        TunnelMode::Reverse => {
            let local = record.local.parse::<SocketAddr>().map_err(|_| {
                ConduitError::UnsupportedTunnelRestart {
                    id: record.id.clone(),
                    reason: format!("stored local target is invalid: {}", record.local),
                }
            })?;

            Ok(PreparedTunnelConfig::Reverse(expose::types::ExposeConfig {
        ...
        TunnelMode::Forward => {
            let bind_addr = record
                ...
                .map_err(|_| ConduitError::UnsupportedTunnelRestart {
                    id: record.id.clone(),
                    reason: format!(
                        "stored bind address is invalid: {}",
                        record.bind_addr.as_deref().unwrap_or("unknown")
                    ),
                })?;
            let mut mappings = Vec::with_capacity(record.forward_mappings.len());
            for mapping in &record.forward_mappings {
                mappings.push(ForwardMapping::parse(mapping).map_err(|_| {
                    ConduitError::UnsupportedTunnelRestart {
                        id: record.id.clone(),
                        reason: format!("stored forward mapping is invalid: {}", mapping),
                    }
                })?);
            }

            Ok(PreparedTunnelConfig::Forward(ForwardConfig {
                server: record.server.clone(),
                user: record.user.clone(),
                bind_addr,
                mappings,
